...
roll_input = st.text_input(
    "Enter a roll number from the available data", 
    key="roll_input", 
...
if not roll_input:
    st.info("Please enter a roll number to view the GPA prediction.")
    st.stop()

roll = roll_input.strip().upper()

# Define paths for the new folder structure
excel_path = os.path.join("data", "excel", f"{roll}_Results.xlsx")
info_path = os.path.join("data", "json", f"{roll}_Info.json")

# --- Data Loading: No scraping, just check for local files ---
try:
    df_result = preprocess_excel(excel_path)
except FileNotFoundError:
    st.error(f"📂 Data for roll number '{roll}' is not available.")
    st.info("Please make sure the corresponding Excel file exists in the 'data/excel' folder.")
    st.stop()
...
try:
    ...
    st.warning("⚠️ Profile info (.json file) not found. Displaying basic information.")
